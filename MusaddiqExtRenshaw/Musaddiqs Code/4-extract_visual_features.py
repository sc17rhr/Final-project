...
def _calculate_rank(orderedset):

  set={}
  rank = 0

  for coord in orderedset:
  ...
  return[set[coord] for coord in orderedset]


def _get_comparison_set(positions):
      ...
      for obj in positions:
          if obj != 'gripper':
              ...
              if [x, y, z] not in IniCoordinates:
                  IniCoordinates.append([x, y, z])

      grouping = 0
      for NoPositions in IniCoordinates:
          count = 0
          for axis in NoPositions:
              if count == 0:
                  individualCoor.append([axis,'x',grouping])
              if count == 1:
                  individualCoor.append([axis,'y',grouping])
              if count == 2:
                  individualCoor.append([axis,'z',grouping])
              count += 1
          grouping += 1

          individualCoor = sorted(individualCoor, key=lambda x: x[0], reverse = True)

      return individualCoor


def _get_set_order(coordinateList):
    ...
    for axis in coordinateList:
        if axis[1] == "x":
            Xref.append([axis[0],axis[2]])
            Xcoordlist.append(axis[0])
        if axis[1] == 'y':
            Yref.append([axis[0],axis[2]])
            Ycoordlist.append(axis[0])
        if axis[1] == 'z':
            Zref.append([axis[0],axis[2]])
            Zcoordlist.append(axis[0])

            #find the max value in an axis
            #maxX = Xref[0][0]
            #maxY = Yref[0][0]
            #maxZ = Zref[0][0]


            #rank the list of co-ordinates in each plane
            rankX = list(_calculate_rank(tuple(Xcoordlist)))
            rankY = list(_calculate_rank(tuple(Ycoordlist)))
            rankZ = list(_calculate_rank(tuple(Zcoordlist)))

            for i in range(0,len(rankX)):
                #print i
                Xref[i][0] = rankX[i]

            for i in range(0,len(rankY)):
                Yref[i][0] = rankY[i]

            for i in range(0,len(rankZ)):
                Zref[i][0] = rankZ[i]
...
def _get_ranked_coordinates(x, y, z):
    ...
    for i in range(0,len(x)):
        for j in range(0,len(y)):
            for k in range(0,len(z)):
                ...
                #re
                if z[k][1] == x[i][1] and z[k][1] == y[j][1]:
                    pairedX = (x[i][0]+1)*10
                    pairedY = (y[i][0]+1)*10
                    superlatives.append([pairedX,pairedY])

    return superlatives


#gathers compiled ranked coordinates
def _get_superlatives(positions):

    comparisonset = _get_comparison_set(positions)

    #print comparisonset
    x, y, z = _get_set_order(comparisonset)
    #print x, y, z
    superlatives = _get_ranked_coordinates(x, y, z)
...
def _get_locations2(positions):#not relevant
    locations = []
    mov_obj = None
    for obj in positions:
        if obj != 'gripper':
...
def _get_distances(positions):
    distances = []
    mov_obj = None
    for obj in positions:# checks for object to be moved
        if obj != 'gripper': # if object isnt gripper
            if positions[obj]['moving']:
                mov_obj = obj
                break
    if mov_obj != None:#if there is an object to be moved
        x1 = positions[mov_obj]['x']#find x and y
        y1 = positions[mov_obj]['y']
#        print "x1 in positions------------", x1
#        print "y1 in positions------------", y1
#        print "move object---------------", mov_obj
        # z1 = positions[mov_obj]['z']
        for obj in positions:#from all objects in scene
            if obj != 'gripper' and obj != mov_obj:#if object isnt moving or gripper
                x2 = positions[obj]['x']
                y2 = positions[obj]['y']
#                print "object----------------", obj
#                print "x2 in pos------------------", x2
...
def _get_aggregates(positions):
    ...
    for collection in cleaned_set_of_collinear_data:
        ...
        #print int(str(direction[0])+str(direction[1]))

        single_aggregate_dataset = [int(str(direction[0])+str(direction[1]))]
...
def _get_aggregates2(positions):
    ...
    collinear_objects = _get_collinear_objects(positions)
    cleaned_set_of_collinear_data = _remove_duplicates_and_non_sequential_locations(collinear_objects)
    #print cleaned_set_of_collinear_data
    # now i have all aggregates that are simply rows
    # how can i get them sorted by colors, so i can know which agg
    # has objects that share colors or shape or nothing
    # get direction here
    for collection in cleaned_set_of_collinear_data:
        last = len(collection) - 1
        ...
        direction = _get_direction_of_aggregate(collection[0][2][0], collection[0][2][1], collection[0][2][2],
                                                collection[last][2][0], collection[last][2][1], collection[last][2][2])
        single_aggregate_dataset = [collection, int(str(direction[0])+str(direction[1]))]
        formatted_aggregate_datasets.append(single_aggregate_dataset)
...
def _get_collinear_objects(positions):
    ...
    #  in this next section i can check for objects and colors, but how do i arrange them
    for obj in positions:
        ...
        if current_obj is not None:
            ...
            #print objects_on_the_same_y_and_x_axis
...
def _cluster_data(X, GT, name, n):# gaussian mixture model
    print name
    for i in range(5):
        print '#####', i
        n_components_range = range(3, n)#chosen by bic
        cv_types = ['spherical', 'tied', 'diag', 'full']
        lowest_bic = np.infty #infinity pos
...
def _append_data5(data, X_, unique_, GT_, mean, sigma):
    for i in data:
        if i not in unique_:
            unique_.append(i)
        # print i,len(i)
...
def _pretty_plot_superlatives():
    clusters = {}
    XY = X_superlative * 180 / 9 + 10
    # print GT_locations
    Y_, best_gmm = pickle.load(
        open(
            '../Datasets/Dataset1/results/superlatives_clusters.p',
            "rb"))
    print XY
    for x, val in zip(XY, Y_):
        if val not in clusters:
    ...
    for c in clusters:
        ...
        plt.axis("off")
        plt.savefig(
            '../Datasets/Dataset1/results/superlatives/' + str(
                c) + '_cluster.png')
        # avg_images[c] = cv2.imread(dir_save+'avg_'+str(c)+".png")


def _svm(x, y, x_test, y_test):#support vector machine
...
#         open('/media/psf/Home/Desktop/New Dataset/results/aggregates_clusters.p',
#              "rb"))
...
#         ax.set_zlabel("z")
#         fig.savefig(
#             '/media/psf/Home/Desktop/New Dataset/results/' + str(cluster) + '_cluster.png')
#         plt.show()
...
for scene in scenes:
    ...
    #print _get_superlatives(positions)
#    VF['superlatives'] = _get_superlatives(positions)
...
for test in range(1):
    ...
    X_aggregates = []#first except pretty plot line 680?
    GT_aggregates = []#first inst
    X_aggregates_t = []#fist isnt
    GT_aggregates_t = []#first inst
    unique_aggregates = []#first inst
        ...
        if c != test:
            for scene in data:
                ...
                X_aggregates, unique_aggregates, GT_aggregates = _append_data4(_get_aggregates(positions),
                                                                               ...
                                                                               GT_aggregates, [0, 0],
                                                                               [[0, 0], [0, 0]])
                X_superlative, unique_superlative, GT_superlative = _append_data5(_get_superlatives(positions),
                                                                               X_superlative,
                                                                               unique_superlative,
                                                                               GT_superlative, [0, 0],
                                                                               [[.4, 0], [0, .4]])
#---------------------------------------------------------------------------------------------
    ...
    _cluster_data(X_superlative, GT_superlative, "superlatives", 9)
#    _svm(X_directions, GT_directions_t, X_directions_t, GT_directions_t)#support vector machine
    _svm(X_aggregates, GT_aggregates, X_aggregates_t, GT_aggregates_t)
#    _pretty_plot_aggregates()
#    _pretty_plot_superlatives()
